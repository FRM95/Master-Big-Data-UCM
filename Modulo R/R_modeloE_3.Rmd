...
personas.ampliado[5] = df.adicional
# FIN SOLUCION
```
...
#En la frase: "reemplazar en `state.df` por `NA` todos los valores de la columna `Illiteracy` para aquellos estados cuya área (`Area`) supere en más de un 50 % a la superficie media de los estados", se ha interpretado que se debe calcular previamente la media del AREA de todos los estados (guardandola en mean.area) y no del INGRESO como dice el enunciado. 

mean.area = mean(state.df$Area)
mascara.mayor.area = state.df$Area > 1.5*mean.area

for(i in 1:50){
    if(mascara.mayor.area[i] == TRUE){
        state.df[i,"Illiteracy"] = NA
    }
}

state.df[nrow(state.df)+1,] = apply(state.df, MARGIN = 2, min, na.rm = TRUE)
rownames(state.df)[51] <- "Minimos"

aniadeProporciones <- function(df){
 minRow = df["Minimos",]
 df2 = matrix(ncol= ncol(df), nrow=nrow(df))
 for(i in 1:length(minRow)){
     df2[,i] = as.numeric(df[,i])/as.numeric(minRow[i])
 }
 df2 = data.frame(df2)
 colnames(df2) = paste(colnames(df),"prop",sep = "_")
...
rownames(state.df.abb)[1:nrow(state.df.abb)-1] <- state.abb

division = factor(c(as.character(state.division), NA))
state.df.abb$Division <- (division)

income.zonas = tapply(state.df.abb$Income,state.df.abb$Division,FUN = max,na.rm = TRUE)

res <- vector()
for (i in state.df.abb$Division) {
  val <- income.zonas[i]
  res <- c(res,val)
  as.numeric(res)
}
state.df.abb$Division.max.income <- res

state.df.poor = subset(state.df.abb, state.df.abb$Income<0.8*Division.max.income, select = c(Population,`Life Exp`,Division.max.income,Income))
# FIN SOLUCION
```
...
life.exp = state.df.poor$`Life Exp`
if(is.null(life.exp)){
  life.exp = state.df.poor$Life.Exp
}
stopifnot(round(mean(life.exp, na.rm = T), 2) == 70.81)
```
...
filtrarSepalLengthPetalWidth <- function(df, umbralSepalLength, umbralPetalWidth){
	df %>% filter(df["Sepal.Length"] >= umbralSepalLength & df["Petal.Width"] >= umbralPetalWidth)
}
